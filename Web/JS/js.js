...
function SelectRow(Row){
	...
	if(Row!=Last_Row){
		...
		for(i=0;i<Row.cells.length;i++){		
			Row.cells[i].className="SelectRow2";
		}	
	} 
...
function SearchHelper(url1,url2)
{
	var qs="";
	try{
		var url_1 = url1.split(',');
		var url_2 = url2.split(',');
		alert(url_1.length);
		alert(url_2.length);
		if(url_1.length==url_2.length)
		{
			...
			{ 
				...
				var q=Trim(document.getElementById(url_2[i]).value);
				alert(q);
				if(q!="")
					qs=qs+url_1[i]+"="+q+"&";
			}
