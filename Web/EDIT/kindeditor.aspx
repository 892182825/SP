<%@ Page Language="C#" AutoEventWireup="true" CodeFile="kindeditor.aspx.cs" Inherits="EDIT_kindeditor" %>
...
KE.lang = {
	source : '<%=GetTran("005721", "切换模式")%>',
	undo : '<%=GetTran("005722", "后退(Ctrl+Z)")%>',
	redo : '<%=GetTran("005723", "前进(Ctrl+Y)")%>',
	cut : '<%=GetTran("005724", "剪切(Ctrl+X)")%>',
	copy : '<%=GetTran("005725", "复制(Ctrl+C)")%>',
	paste : '<%=GetTran("005726", "粘贴(Ctrl+V)")%>',
	plainpaste : '<%=GetTran("005727", "粘贴为无格式文本")%>',
	wordpaste : '<%=GetTran("005728", "从Word粘贴")%>',
	selectall : '<%=GetTran("004198", "全选")%>',
	justifyleft : '<%=GetTran("005729", "左对齐")%>',
	justifycenter : '<%=GetTran("005730", "居中")%>',
	justifyright : '<%=GetTran("005731", "右对齐")%>',
	justifyfull : '<%=GetTran("005732", "两端对齐")%>',
	insertorderedlist : '<%=GetTran("001195", "编号")%>',
	insertunorderedlist : '<%=GetTran("005733", "项目符号")%>',
	indent : '<%=GetTran("005734", "增加缩进")%>',
	outdent : '<%=GetTran("005735", "减少缩进")%>',
	subscript : '<%=GetTran("005736", "下标")%>',
	superscript : '<%=GetTran("005737", "上标")%>',
	title : '<%=GetTran("000781", "标题")%>',
	fontname : '<%=GetTran("005814", "字体")%>',
	fontsize : '<%=GetTran("005738", "文字大小")%>',
	textcolor : '<%=GetTran("005739", "文字颜色")%>',
	bgcolor : '<%=GetTran("005740", "文字背景")%>',
	bold : '<%=GetTran("005741", "粗体")%>',
	italic : '<%=GetTran("005742", "斜体")%>',
	underline : '<%=GetTran("005743", "下划线")%>',
	strikethrough : '<%=GetTran("005744", "删除线")%>',
	removeformat : '<%=GetTran("005745", "删除格式")%>',
	image : '<%=GetTran("005746", "图片")%>',
	flash : '<%=GetTran("005747", "插入Flash")%>',
	media : '<%=GetTran("005748", "插入多媒体")%>',
	table : '<%=GetTran("005749", "插入表格")%>',
	hr : '<%=GetTran("005750", "插入横线")%>',
	emoticons : '<%=GetTran("005751", "插入表情")%>',
	link : '<%=GetTran("005752", "超级连接")%>',
	unlink : '<%=GetTran("005753", "取消超级连接")%>',
	fullscreen : '<%=GetTran("005754", "全屏显示")%>',
	about : '<%=GetTran("005755", "关于")%>',
	print : '<%=GetTran("001149", "打印")%>',
	yes : '<%=GetTran("000434", "确定")%>',
	no : '<%=GetTran("001614", "取消")%>',
	close : '<%=GetTran("000019", "关闭")%>',
	editImage : '<%=GetTran("005756", "图片属性")%>',
	deleteImage : '<%=GetTran("005757", "删除图片")%>',
	editLink : '<%=GetTran("005758", "超级连接属性")%>',
	deleteLink : '<%=GetTran("005753", "取消超级连接")%>',
	noColor : '<%=GetTran("005759", "无填充颜色")%>',
	invalidImg : '<%=GetTran("005760", "请输入有效的URL地址。\n只允许jpg,gif,bmp,png格式。")%>',
	invalidMedia : '<%=GetTran("005761", "请输入有效的URL地址。\n只允许swf,flv,mp3,wav,wma,wmv,mid,avi,mpg,asf,rm,rmvb格式。")%>',
	invalidWidth : '<%=GetTran("005762", "宽度必须为数字。")%>',
	invalidHeight : '<%=GetTran("005763", "高度必须为数字。")%>',
	invalidBorder : '<%=GetTran("005764", "边框必须为数字。")%>',
	invalidUrl : '<%=GetTran("005765", "请输入有效的URL地址。")%>',
	pleaseInput : '<%=GetTran("005766", "请输入内容")%>',
	cutError : '<%=GetTran("005786", "您的浏览器安全设置不允许使用剪切操作，请使用快捷键(Ctrl+X)来完成。")%>',
	copyError : '<%=GetTran("005788","您的浏览器安全设置不允许使用复制操作，请使用快捷键(Ctrl+C)来完成。") %>',
	pasteError : '<%=GetTran("005789", "您的浏览器安全设置不允许使用粘贴操作，请使用快捷键(Ctrl+V)来完成。")%>'
};

KE.scriptPath = (function() {
	var elements = document.getElementsByTagName('script');
	for (var i = 0, len = elements.length; i < len; i++) {
		if (elements[i].src && elements[i].src.match(/kindeditor[\w\-\.]*\.aspx/)) {
