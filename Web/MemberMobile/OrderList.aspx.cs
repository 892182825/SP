...
public partial class OrderList : BLL.TranslationBase
{
    ...
    public void Bind()
    {
       ...
       DataTable  dt= DBHelper.ExecuteDataTable("select top 1  OrderID, TotalMoney   from  memberorder where  Number='"+number+"' and  DefrayState=1 and isactive=0 and ordertype  in (22,23,24) order by ID  ");
        string orderid = "";
        double ttm = 0;
        string st = " ";
        if (dt != null && dt.Rows.Count > 0)
        {
            ...
            acthtml = " <input type= 'button' onclick= 'showbuy()' class='actv'   value='激活' />";
            st = "未激活";
        }
        else {
            st = "已激活";
...
</div><div class='dsc2' >" + acthtml+ "<p class='p1'>&nbsp;" + usdcn + @"%</p><p class='p3'></p><p class='p2'> "+ st +" </p>   </div>  </ li > ";
        } 
        html += " </ul>";
