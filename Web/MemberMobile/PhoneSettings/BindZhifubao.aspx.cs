...
public partial class MemberMobile_PhoneSettings_BindZhifubao : BLL.TranslationBase
{
    ...
    protected void btn_submit_Click(object sender, EventArgs e)
    {
        ...
        if (Member != null)
        {
            var value = updateMember(Member.ToString(), zhifubao);
            if (value > 0)
            {
                ClientScript.RegisterStartupScript(this.GetType(), "", "<script>alert('支付宝修改成功');</script>", false);
                Response.Redirect("SettingsIndex.aspx?res=success&&type=fanhui");
                txtzhifubao.Text = zhifubao;
            }
            else
            {
                ClientScript.RegisterStartupScript(this.GetType(), "", "<script>alert('支付宝修改失败');</script>", false);
                txtzhifubao.Text = "";
            }
        }
    }
    ...
    {
        var returnvalue = 0;
        var sql = @"update memberinfo set zhifubao=@zhifubao where Number=@Number";
        SqlParameter[] para = {
                                      new SqlParameter("@zhifubao",SqlDbType.VarChar),
