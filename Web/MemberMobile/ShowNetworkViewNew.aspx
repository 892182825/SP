<%@ Page Language="C#" AutoEventWireup="true" CodeFile="ShowNetworkViewNew.aspx.cs" Inherits="Member_ShowNetworkViewNew" %>
...
<script type="text/javascript">
        ...
        obj = $(".tr");
            ...
            try
            {
                fObj.animate({ "left": thisObj.children("td").eq(0).children().children(".img").offset().left + 15 + "px","top": minTr.attr("offsetTop")+51}, 100, function() {
            ...
            {
                fObj.animate({ "left": thisObj.children("td").eq(0).children().children().children(".img").offset().left + 15 + "px","top": minTr.attr("offsetTop")+51}, 100, function() {
                htmlStr += "<tr style='height:18px'><th>代数</th><th>级别</th><th>新个分数</th><th>新网分数</th><th>新网人数</th><th>总网人数</th><th>总网分数</th></tr>";
                    while (minIndex <= maxIndex) {
                        if (thisObj.css("display") != "none") {
                            objTd = obj.eq(minIndex).children("td");
   ...
   // debugger;
	...
	switch(flag)
	{
	    ...
	    //    alert('对编号：'+bianhao+'下级注册');
	        window.location.href="registerMember.aspx?type=1&bh="+bianhao;
	        break;
...
function JSNET(thisObj,bianhao,tree,flag,qishu,cw)
{
    ...
    else
    {
        ...
        var bhCw = AjaxClass.GetLogoutCw(bh,qishu,true).value;
        var loginCw = AjaxClass.GetLogoutCw(loginBh,qishu,true).value;
...
<a style="display:none" href="javascript:window.history.back()"><u><%=GetTran("000421", "返回")%><%=GetTran("000846", "以下图中深红色代表已审核新增人员，淡红色代表未审核新增人员")%></u></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                               ...
                                               <asp:Button ID="Button4" runat="server" Text="展开" CssClass="anyes" 
                                                ...
                                                <asp:Button ID="Button8" runat="server" Text="伸缩" Enabled="false" CssClass="anyes" />			&nbsp;&nbsp;&nbsp;
