...
public partial class Member_MemberCash : BLL.TranslationBase
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (Session["xfcz"] != null && Session["xfcz"] != "")
            {
                getzf();
 ...
 //}
    ...
    private void GetActMoney()
    {
        ...
        if (dt_one.Rows.Count > 0)
        {
            ...
            kymoney.Text = dt_one.Rows[0]["xf"].ToString();
            if (ipn != "")
            {
                string quhao = "";
	...
	catch 
	{
        ...
        System.Web.UI.ScriptManager.RegisterStartupScript(Page, GetType(), "success2", "alertt('请先去钱包完善信息,如果已实名请联系管理员！');", true);
        return;
	}
...
{
            {"out_trade_no", Session["xfcz"].ToString()}   
};
        ...
        if (zt == "SUCCESS")
        {
            ...
            {
                int ret = 0;
                string sql = "update memberinfo set TotalRemittances=TotalRemittances+" + skje + " where number='" + Session["Member"].ToString() + "'";
                int cg = DBHelper.ExecuteNonQuery(tran,sql);

                ret = DBHelper.ExecuteNonQuery(tran, "INSERT INTO MemberAccount(Number,HappenTime,HappenMoney,BalanceMoney,Direction,SfType,KmType,Remark)SELECT j.number,GETutcDATE()," + skje + " ,j.TotalRemittances-j.TotalDefray,0,0,2,'成功充值FTC" + skje + "'from memberinfo j WHERE j.Number='" + Session["Member"].ToString() + "'");
                ...
                if (cg > 0 && ret > 0)
                {
                    ...
                    ClientScript.RegisterStartupScript(this.GetType(), "", "<script>$('#tiaoz').show();document.getElementById('tiaoz').href = 'SHJF.aspx'; ;alertt('充值申请成功，已经到账请查收,如没到账请联系客服。');</script>", false);
        ...
        else
        {
            ...
            ClientScript.RegisterStartupScript(this.GetType(), "", "<script>$('#tiaoz').show();document.getElementById('tiaoz').href = 'MemberCZXF.aspx'; ;alertt('正在查询订单支付情况，请稍后查看余额');</script>", false);
            Session["xfcz"] = "";
            //    this.money.Text = "";
            //    this.remark.Text = "";
            //}
            ...
            Dictionary<String, Object> data = new Dictionary<String, Object>
{
            ...
            {"return_url", "http://zd.factorde.com/MemberMobile/MemberCZXF.aspx"}
