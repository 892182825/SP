...
public partial class Member_MemberCash : BLL.TranslationBase
{
 ...
 //}
	...
	{	        
                    ...
                    //rmoney.Text = rspp;
                     //stJson["data"]["normal_balance"].ToString();
                    if (Convert.ToDouble(stJson["data"]["normal_balance"].ToString()) > 0)
                    {
                        usdtye = stJson["data"]["normal_balance"].ToString();
                    }
                    else
                    {
                        ...
                        myDct.Add("uid", uid);
                        myDct.Add("symbol", "USDTERC20");
                        myDct.Add("app_id", "98bb076f4d2585c38e5ffaa6921779e0");
                        myDct.Add("time", GetTimeStamp());

                         signj = PublicClass.GetSignContents(myDct, "d6462b1fe40f38392d3f6d8d4a52e9e2");
                         jsonS = MD5(signj);

                        myDct.Add("sign", jsonS);

                         rspp = PublicClass.GetFunction(postdz, myDct);
                        JObject sjzj = JObject.Parse(rspp);
                        if (Convert.ToDouble(sjzj["data"]["normal_balance"].ToString()) > 0)
                        {
                            usdtye = sjzj["data"]["normal_balance"].ToString();
                        }
                        else
                        {

                            Dictionary<String, String> mdta = new Dictionary<String, String>();


                            mdta.Add("uid", uid);
                            mdta.Add("symbol", "TUSDT");
                            mdta.Add("app_id", "98bb076f4d2585c38e5ffaa6921779e0");
                            mdta.Add("time", GetTimeStamp());

                            signj = PublicClass.GetSignContents(mdta, "d6462b1fe40f38392d3f6d8d4a52e9e2");
                            jsonS = MD5(signj);

                            mdta.Add("sign", jsonS);

                            rspp = PublicClass.GetFunction(postdz, mdta);
                            JObject shuiji = JObject.Parse(rspp);
                            if (Convert.ToDouble(shuiji["data"]["normal_balance"].ToString()) > 0)
                            {
                                usdtye = shuiji["data"]["normal_balance"].ToString();
                            }
