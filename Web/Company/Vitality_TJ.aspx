<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Vitality_TJ.aspx.cs" Inherits="Vitality_TJ" %>
...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript">
        ...
        function getData(param,backM)
        {
            ...
            xmlHttp.open("get","Vitality_TJ.aspx?"+param,false);
        ...
        function getAsyData(param,backM)
        {
            ...
            xmlHttp.open("get","Vitality_TJ.aspx?"+param,true);
            xmlHttp.onreadystatechange=backM;
            xmlHttp.send(null);
...
//            showPageGray();
        ...
        function getStr(vcount) {
             
          var  number= 0;
        if(vcount<1 )vcount=1;
        if(vcount>255)vcount=256;
        if (vcount > 128) number = 16711680 - ((vcount - 128) * 256 * 256)
        else number = 16777215 - vcount * 512


        var j16 = v10toX(16, number);
            ...
            if(xmlHttp.readyState==4)
            {
                if(xmlHttp.status==200)
                {
                    ...
                    for(var i=0;i<arrRow.length;i++)
                    {
                        ...
                        var vcount = arrRow[i].getElementsByTagName("vitalitycount")[0].firstChild.nodeValue; //color

                        if (vcount > 128) tdcolor = "#fff";
                      else  tdcolor="#000";
...
//                        };
                   //  this.style.backgroundColor=_color;
                        ...
                        trobj.style.backgroundColor =   _color;
                       // trobj.setAttribute("style","background-color:"+_color)
                        //会员编号td
                        ...
                        tdobj.onclick=function(e)
                        {
                            ...
                            if(md.nodeName.toLowerCase()=="td")
                                window.location.href="Vitality_TJ.aspx?number="+this.parentNode.attributes["parentId"].value.replace("tr","")+"&thNumber="+this.childNodes[this.childNodes.length-1].nodeValue.split(" ")[0]+"&EndNumber=<%=EndNumber%>&ExpectNum="+_document.getElementById("DDLQs").value;
                        };
                        ...
                        //安置人td
                        if(arrRow[i].getElementsByTagName("Placement").length==1)
                        {
                            var Placement="";try{Placement=arrRow[i].getElementsByTagName("Placement")[0].firstChild.nodeValue}catch(ee){};
...
//             }
                ...
                if (xmlHttp.status == 200) {
                    if (xmlHttp.responseText == "OK") {
                        ...
                        if (tdhtml.indexOf(__tempImg) == -1)
                            ...
                            _document.getElementById(trid).getElementsByTagName("td")[0].innerHTML = tdhtml.replace("<SPAN style=\"BACKGROUND-IMAGE: url(" + __tempImg + "); BACKGROUND-REPEAT: no-repeat\">　&nbsp;</SPAN>", "");
                        */
            ...
            if(xmlHttp.readyState==4)
            {
                if(xmlHttp.status==200)
                {
                    if(xmlHttp.responseText.indexOf("OK")!=-1)
                    {
                        ...
                        if(document.getElementById("mColor").innerHTML!="取消团队变色")
                        {
                            for(var i=1;i<arrtrid.length;i++)
                            {
                                document.getElementById("tr"+arrtrid[i]).getElementsByTagName("td")[0].style.color="blue";
                            }
                        }
                        else
                        {
                            for(var i=1;i<arrtrid.length;i++)
                            {
                                document.getElementById("tr"+arrtrid[i]).getElementsByTagName("td")[0].style.color="";
                            }
                        }
        ...
        function linkCY()
        {
            window.location.href="MemberNetMap.aspx?net=tj&bianhao=<%=ThNumber %>&SelectGrass="+_document.getElementById("DDLQs").value;
        }        
          
        function linkAZ()
        {
            window.location.href="SST_AZ.aspx?number=<%=GetNumberParent_AZ(ThNumber) %>&thNumber=<%= ThNumber%>&ExpectNum="+_document.getElementById("DDLQs").value;
            //window.location.href="SST_AZ.aspx?ExpectNum="+_document.getElementById("DDLQs").value;
        } 
...
//        var grayDiv;
//        function showPageGray()
//        {
//            if(grayDiv==null)
//            {
//                grayDiv=window.top._document.createElement("div");
//            }
//            
//            grayDiv.style.position="absolute";
//            grayDiv.style.left="0px";
//            grayDiv.style.top="0px";
//            grayDiv.style.width=_document.documentElement.scrollWidth+"px";
//            grayDiv.style.height=_document.documentElement.scrollHeight+"px";
//            grayDiv.style.backgroundColor="black";
//            grayDiv.style.filter="alpha(opacity=60)";
//            grayDiv.style.opacity="0.6";
//            
//            window.top._document.body.appendChild(grayDiv);
//        }
//        
//        function hidPageGray()
//        {
//            if(grayDiv!=null)
//            {
//                window.top._document.body.removeChild(grayDiv);
//            
//            }
//        }
...
<body style="font-size:10pt" oncontextmenu="return false;" >
    ...
    <div style="padding-top:20px;padding-left:20px">
       ...
       <input type="button"  value='<%=GetTran("000420", "常用") %>' class="anyes" style="display:none;"  onclick="linkCY()" />
       <input type="button" value='<%=GetTran("000667","推荐") %>' class="anyes" style="display:none;" disabled >
        
       <input type="button"  value='<%=GetTran("007457","转到安置") %>' style="display:none;" class="anyes" onclick="linkAZ()">
       <br />
       <br />
       <span style="color:rgb(0,61,92)">
